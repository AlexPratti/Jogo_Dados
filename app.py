...
import random
import os
import json  # Importado para salvar os dados permanentemente
...
ARQUIVO_DADOS = "dados_figuras.json"

# Funções para ler e gravar os dados de forma permanente
def carregar_dados():
...
# 1. GERENCIAMENTO DO DICIONÁRIO DE FIGURAS (Carrega do arquivo JSON)
if "dicionario_figuras" not in st.session_state:
    st.session_state.dicionario_figuras = carregar_dados()
    ...
    if st.button("Adicionar ao Jogo", use_container_width=True):
        if novo_id and novo_nome_real:
            if novo_id not in st.session_state.dicionario_figuras:
                ...
                salvar_dados(st.session_state.dicionario_figuras)
...
with aba_editar:
    ...
    st.caption("Os nomes são atualizados assim que alterados.")
    mudou = False
    
    for arquivo_id, nome_atual in list(st.session_state.dicionario_figuras.items()):
        novo_nome = st.text_input(
            f"Nome para {arquivo_id}.png:", value=nome_atual, key=f"edit_{arquivo_id}"
        ).strip()
...
# Mostra informações sobre o ciclo atual de imagens
